...
def infer_logic_rules(model, processor, train_positive, train_negative, principle):
    ...
    return _generate(model, processor, messages, max_new_tokens=128)
...
def evaluate_llm(model, processor, test_images, logic_rules, principle):
    ...
    for image, label, img_id in test_images:
        ...
        response = _generate(model, processor, messages, max_new_tokens=32)
...
def _run_baseline(model_id, model_name, data_path, img_size, principle,
    ...
    model, processor = load_llava_ov15_model(model_id)
...
def run_llava_ov15_4B_baseline(data_path, img_size, principle, batch_size, device, img_num, epochs, start_num, task_num):
    return _run_baseline("lmms-lab/LLaVA-OneVision-1.5-4B-Instruct", "LLaVA-OV-1.5-4B",
                          data_path, img_size, principle, batch_size, img_num, start_num, task_num)


def run_llava_ov15_8B_baseline(data_path, img_size, principle, batch_size, device, img_num, epochs, start_num, task_num):
    return _run_baseline("lmms-lab/LLaVA-OneVision-1.5-8B-Instruct", "LLaVA-OV-1.5-8B",
                          data_path, img_size, principle, batch_size, img_num, start_num, task_num)
